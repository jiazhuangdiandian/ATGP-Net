...
for i in range (1):
   ...
   resume = 'Tru'   #是否继续上一次中断的代码
   val_num =1   #数据集划分随机数种子，暂时不要修改

#####################################################################################
models_name = "RCM_wave_M_or_K_UNet_Super_D__sig_hwconv_1_KM_ScConv_2"  #   UNet UNet_2plus Att_UNet     UC_TransNet   ResU_Net   UTNet  PAttUNet   DCSAU_Net  TransAttUnet  ---------   OUNet
dataset_name = "busi"    #"CVC-ClinicDB" "isic2018" "buli"  "Dataset B"   Dataset_B_90  busi Kvasir
#####################################################################################
...
if models_name == 'RCM_wave_M_or_K_UNet_Super_D__sig_hwconv_1_KM_ScConv_2':   #OUNet_UOS   OUNet OUNet_O
    archs_name = archs3
else:
    archs_name = archs
...
def parse_args():
    ...
    parser.add_argument('--name', default='{}_{}{}_4'.format(models_name, dataset_name____, val_num),
